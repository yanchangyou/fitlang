...
final class JsonSchemaStatusWidget extends EditorBasedStatusBarPopup {
  public static final String ID = "FitJsonSchemaSelector";
  ...
  JsonSchemaStatusWidget(@NotNull Project project, @NotNull CoroutineScope scope) {
    ...
    fit.jetbrains.jsonSchema.extension.JsonWidgetSuppressor.EXTENSION_POINT_NAME.addChangeListener(this::update, this);
  }
  ...
  public static boolean isAvailableOnFile(@NotNull Project project, @Nullable VirtualFile file) {
    ...
    List<fit.jetbrains.jsonSchema.extension.JsonSchemaEnabler> enablers = fit.jetbrains.jsonSchema.extension.JsonSchemaEnabler.EXTENSION_POINT_NAME.getExtensionList();
  ...
  private static WidgetStatus getWidgetStatus(@NotNull Project project, @NotNull VirtualFile file) {
    ...
    if (ContainerUtil.exists(fit.jetbrains.jsonSchema.extension.JsonWidgetSuppressor.EXTENSION_POINT_NAME.getExtensionList(), s -> s.isCandidateForSuppress(file, project))) {
  ...
  private @NotNull WidgetState doGetWidgetState(@NotNull VirtualFile file, boolean isJsonFile) {
    ...
    fit.jetbrains.jsonSchema.JsonSchemaMappingsProjectConfiguration userMappingsConfiguration = JsonSchemaMappingsProjectConfiguration.getInstance(myProject);
    ...
    schemaFile = ((fit.jetbrains.jsonSchema.impl.JsonSchemaServiceImpl)service).replaceHttpFileWithBuiltinIfNeeded(schemaFile);
    ...
    fit.jetbrains.jsonSchema.extension.JsonSchemaFileProvider provider = service.getSchemaProvider(schemaFile);
    if (provider != null) {
      ...
      String kind =
        !useRemoteSource && (provider.getSchemaType() == fit.jetbrains.jsonSchema.extension.SchemaType.embeddedSchema || provider.getSchemaType() == fit.jetbrains.jsonSchema.extension.SchemaType.schema)
  ...
  private void scheduleSuppressCheck(@NotNull VirtualFile file, @NotNull ProgressIndicator globalProgress) {
    Runnable update = () -> {
      if (DumbService.getInstance(getProject()).isDumb()) {
        ...
        boolean suppress = ContainerUtil.exists(fit.jetbrains.jsonSchema.extension.JsonWidgetSuppressor.EXTENSION_POINT_NAME.getExtensionList(),
  ...
  private static @NotNull @Nls String getPresentableNameForFile(@NotNull VirtualFile schemaFile) {
    if (schemaFile instanceof HttpVirtualFile) {
      return new fit.jetbrains.jsonSchema.extension.JsonSchemaInfo(schemaFile.getUrl()).getDescription();
    }

    String nameWithoutExtension = schemaFile.getNameWithoutExtension();
    if (!fit.jetbrains.jsonSchema.extension.JsonSchemaInfo.isVeryDumbName(nameWithoutExtension)) return nameWithoutExtension;
  ...
  private static @Tooltip String createMessage(final @NotNull Collection<? extends VirtualFile> schemaFiles,
    ...
    final List<Pair<Boolean, @Nls String>> pairList = schemaFiles.stream()
      ...
      .map(provider -> Pair.create(fit.jetbrains.jsonSchema.extension.SchemaType.userSchema.equals(provider.getSchemaType()), provider.getName()))
      .toList();
  ...
  private static boolean hasConflicts(@NotNull Collection<VirtualFile> files,
    ...
    List<fit.jetbrains.jsonSchema.extension.JsonSchemaFileProvider> providers = ((JsonSchemaServiceImpl)service).getProvidersForFile(file);
