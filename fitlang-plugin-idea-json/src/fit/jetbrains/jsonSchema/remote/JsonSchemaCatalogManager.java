...
public class JsonSchemaCatalogManager {
    ...
    @NotNull
    private final fit.jetbrains.jsonSchema.remote.JsonSchemaRemoteContentProvider myRemoteContentProvider;
    @Nullable
    private VirtualFile myCatalog = null;
    @NotNull
    private final ConcurrentMap<String, String> myResolvedMappings = ContainerUtil.newConcurrentMap();
    ...
    public VirtualFile getSchemaFileForFile(@NotNull VirtualFile file) {
        ...
        for (Iterator<JsonSchemaCatalogExclusion> it = JsonSchemaCatalogExclusion.EP_NAME.getExtensionList().iterator(); it.hasNext(); ) {
            JsonSchemaCatalogExclusion exclusion = it.next();
        ...
        if (myResolvedMappings.containsKey(name)) {
            ...
            return fit.jetbrains.jsonSchema.remote.JsonFileResolver.resolveSchemaByReference(file, urlString);
        }
