...
import fit.jetbrains.jsonSchema.impl.JsonCachedValues;
...
public final class JsonSchemaCatalogManager {
  ...
  public void startUpdates() {
    fit.jetbrains.jsonSchema.JsonSchemaCatalogProjectConfiguration.getInstance(myProject).addChangeHandler(() -> {
      update();
  ...
  private void update() {
    ...
    myCatalog = !fit.jetbrains.jsonSchema.remote.JsonFileResolver.isRemoteEnabled(myProject) ? null : fit.jetbrains.jsonSchema.remote.JsonFileResolver.urlToFile(DEFAULT_CATALOG);
  }
  ...
  public @Nullable VirtualFile getSchemaFileForFile(@NotNull VirtualFile file) {
    ...
    return fit.jetbrains.jsonSchema.remote.JsonFileResolver.resolveSchemaByReference(file, schemaUrl);
  }
  ...
  public List<fit.jetbrains.jsonSchema.JsonSchemaCatalogEntry> getAllCatalogEntries() {
    if (myCatalog != null) {
      final List<fit.jetbrains.jsonSchema.JsonSchemaCatalogEntry> catalog = fit.jetbrains.jsonSchema.impl.JsonCachedValues.getSchemaCatalog(myCatalog, myProject);
  ...
  public void triggerUpdateCatalog(Project project) {
    fit.jetbrains.jsonSchema.remote.JsonFileResolver.startFetchingHttpFileIfNeeded(myCatalog, project);
  }
  ...
  private static @Nullable String resolveSchemaFile(@NotNull VirtualFile file, @NotNull VirtualFile catalogFile, @NotNull Project project) {
    ...
    List<fit.jetbrains.jsonSchema.JsonSchemaCatalogEntry> schemaCatalog = JsonCachedValues.getSchemaCatalog(catalogFile, project);
  ...
  private static final class FileMatcher {
    private final fit.jetbrains.jsonSchema.JsonSchemaCatalogEntry myEntry;
